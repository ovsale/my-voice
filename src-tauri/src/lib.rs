...
pub(crate) fn save_setting_to_store<T: serde::Serialize>(
    ...
    store.set(storage_key_name, json_value);
    store
        .save()
...
fn start_recording(
    ...
    // Start the recording orchestrator (mic capture → buffer)
    let recording = app.state::<recording::RecordingOrchestrator>();
    if let Ok(mut buf) = recording.audio_buffer().lock() {
        buf.clear();
    }
    let mic_state = app.state::<mic_capture::MicCaptureManager>();
    let selected_mic: Option<String> =
        get_setting_from_store(app, LocalOnlySetting::SelectedMicId, None);
    if let Err(e) = mic_state.capture().start(selected_mic.as_deref()) {
        log::error!("Failed to start mic capture: {e}");
    }
    recording::set_status_pub(&recording, recording::RecordingStatusPub::Recording);
    let _ = app.emit(EventName::RecordingStatusChanged.as_str(), recording::RecordingStatusPub::Recording);
...
fn stop_recording(
    ...
    // Stop mic and run the processing pipeline
    let mic_state = app.state::<mic_capture::MicCaptureManager>();
    mic_state.capture().stop();

    let recording = app.state::<recording::RecordingOrchestrator>();
    recording::trigger_pipeline(app.clone(), &recording);
...
pub fn handle_shortcut_event(app: &AppHandle, shortcut: &Shortcut, event: TauriShortcutEvent) {
    log::info!("handle_shortcut_event called: shortcut={shortcut}, state={:?}", event.state);
...
pub fn run() {
    ...
    builder
        ...
        .setup(|app| {
            ...
            // Initialize mic capture manager — audio data goes into recording orchestrator buffer
            let recording_orchestrator = recording::RecordingOrchestrator::new();
            let buffer = recording_orchestrator.audio_buffer();
            let sample_rate_holder = recording_orchestrator.sample_rate_holder();
            let channels_holder = recording_orchestrator.channels_holder();
            app.manage(recording_orchestrator);

            let mic_capture_manager = MicCaptureManager::new(move |audio_data| {
                // Push audio samples into the recording buffer
                if let Ok(mut buf) = buffer.lock() {
                    buf.extend_from_slice(&audio_data.samples);
                }
                // Store sample rate and channels from first callback
                if let Ok(mut sr) = sample_rate_holder.lock() {
                    *sr = audio_data.sample_rate;
                }
                if let Ok(mut ch) = channels_holder.lock() {
...
pub(crate) fn do_register_shortcuts(app: &AppHandle) -> state::ShortcutRegistrationResult {
    ...
    let try_register = |hotkey: &mut HotkeyConfig,
        ...
        match shortcut_manager.on_shortcut(shortcut, |app_handle, shortcut, event| {
            log::info!("Shortcut event received: {} state={:?}", shortcut, event.state);
